...
function Build {
    # Creates the /dist directory and builds the go files for linux and windows
    Write-Output "Build"
    New-Item -Path dist -ItemType Directory
        ...
        Set-Item -Path env:GOARCH -Value "amd64"
        Invoke-Expression "go build -o dist/${NAME}-windows"
...
function Deploy {
    ...
    Write-Output "Logging into repo - docker login $DOCKER_URL -u $Env:DOCKER_USERNAME -p Env:DOCKER_PASSWORD"    
